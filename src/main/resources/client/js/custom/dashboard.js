...
function calculateCurrentTime(){
    ...
    var hour = String("0" + today.getUTCHours()).slice(-2);
    var min = String("0" + today.getUTCMinutes()).slice(-2);
    today = yyyy + '-' + mm + '-' + dd + 'T' + hour + ':' + min; //Format needed for the date value
    setDate();
}

//Author Jason
//Set the date input to the current time when the modal is first shown
function setDate(){
    field = $(".spending-date")
    field.eq(0).val(today);
}
